...
package "Торговый журнал" {
	usecase "Добавить сделку" as addTrade
	usecase "Посмотреть сделки" as seeTrades
	usecase "Посмотреть журналы" as seeJournals
	usecase "Добавить новый журнал" as addJournal
	usecase "Добавить вариант входа" as addEntryVariant
	usecase "Посмотреть варианты входа" as seeEntryVariant
	usecase "Посмотреть статистику торговли общую" as seeStatistics
	usecase "Посмотреть статистику торговли по журналу" as seeJournalStatistics
	usecase "Настроить экран статистики" as statisticsSettings
}
...
package "Торговый журнал" {
	package "Журнал 1" {
		usecase "Добавить сделку" as addTrade
		usecase "Ввести основные данные" as mainData
		usecase "Ввести дополнительные данные" as addData
	}
}

T --- addTrade
addTrade .> mainData :include
addData .> addTrade :extend

@enduml
```

> [!note]- Добавить сделку (ручной способ)
> 1. Пользователь выбирает торговый журнал с ручным вводом.
> 2. Пользователь вводит основные данные о сделке - пара, позиция, сессия, объем, цена открытия и закрытия сделки, комиссия и стоп-лосс, ставит галочки на пунктах Ганна, выбирает способ входа и оценивает сделку. Некоторые поля высчитываются автоматически - % Ганна, риск, % прибыли / убытка, итог по сделке, результат сделки (+ или -), сумму тиков и сколько всего получено по сделке до вычета комиссий и т.д.
...
package "Торговый терминал MT5" {
	package "Расширение для журнала в  MT5" {
		usecase "Отправить сделку в журнал" as sendTrade
	}
	usecase "Совершение сделки" as Trade
}
...
> [!note]- Добавить сделку (автоматический способ)
...
> [!note]- Посмотреть статистику по журналу
